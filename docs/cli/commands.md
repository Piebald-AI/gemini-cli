...
- **`/resume`**
  ...
  - **Note:** All conversations are automatically saved as you chat - no manual
    saving required. See [Chat Recording](../chat-recording.md) for complete
    details.

- **`/share`**
  - **Description:** Writes the current conversation to a provided Markdown or
    JSON file.
  - **Usage:** `/share file.md` or `/share file.json`. If no filename is
    provided, then the CLI will generate one.
